...
using a single probes: strong gravitational lenses (lenses)
2. We use simulated LSST data.
...
#decide whether to plot a distribution of the lenses data used for testing
hist_plot = False
scatter_plot = False
...
#set mock cosmology
#common parameters we are interested in are Hubble's constant H0, matter density Om0, curvature density Ok0 and
#equation of state parameter w.
if cosmology == "oLCDM":
    H0_mock, Om0_mock, Ok0_mock = 72, 0.3, 0.0 
...
def log_prior(theta):
    if cosmology == "oLCDM":
        ...
        h0, om, ok, w = theta
        if 0. <= h0 <= 150. and 0.05 <= om <= 0.5 and -0.5 <= ok <= 0.5 and -2.5 <= w <= 0.5:
            return 0.0
        else:
            return -np.inf
    else:
...
def log_likelihood(theta, zlens, zsource, ddt_err): 
    if cosmology == "oLCDM":
        h0, om, ok = theta
    ...
    return -0.5*(chi_sq_lenses)

def log_probability(theta, zlens, zsource, ddt_err):
    lp = log_prior(theta)
    if not np.isfinite(lp):
        return -np.inf
    return lp + log_likelihood(theta, zlens, zsource, ddt_err)

#run a maximum likelihood test
if ml_test:
    nll = lambda *args: -log_likelihood(*args)
    if cosmology == "oLCDM":
        initial = np.array([70., 0.27, 0.02])
        soln = minimize(nll, initial, args=(zlens, zsource, ddt_err))
        H0_ml, Om_ml, Ok_ml = soln.x
...
if cosmology == "oLCDM":
    startpos = [70., 0.27, 0.02]  # H0, Om, Ok
    labels = ["H0", "Om", "Ok"]
if cosmology == "owCDM":
    startpos = [70., 0.27, 0.02, -1.1]  # H0, Om, Ok
    labels = ["H0", "Om", "Ok", "w"]
...
if run_mcmc:
    ...
    sampler = emcee.EnsembleSampler(
        nwalkers, ndim, log_probability, args=[zlens, zsource, ddt_err]
    )
...
if save_mcmc:
    ...
    if cosmology == "oLCDM":
        sample_data.to_csv(
            "Samples/Simulation/%sLSSTLike_mock=%s,%s,%s,%s_%ix%i.csv" 
            % (len(zlens), H0_mock, Om0_mock, Ok0_mock, cosmology, nwalkers, nsamples), 
            index=False, header=labels
        )
    elif cosmology == "owCDM":
        sample_data.to_csv(
            "Samples/Simulation/%sLSSTLike_mock=%s,%s,%s,%s,%s_%ix%i.csv" 
            % (len(zlens), H0_mock, Om0_mock, Ok0_mock, w_mock, cosmology, nwalkers, nsamples), 
            index=False, header=labels
        )
...
if load_mcmc:
    if cosmology == "oLCDM":
        flat_samples = pd.read_csv("Samples/Simulation/%sLSSTLike_mock=%s,%s,%s,%s_%ix%i.csv" 
                                   % (len(zlens), H0_mock, Om0_mock, Ok0_mock,
                                      cosmology, nwalkers, nsamples), skiprows=1, header=None
                                  )
    if cosmology == "owCDM":
        flat_samples = pd.read_csv("Samples/Simulation/%sLSSTLike_mock=%s,%s,%s,%s,%s_%ix%i.csv" 
                                   % (len(zlens), H0_mock, Om0_mock, Ok0_mock, w_mock,
                                      cosmology, nwalkers, nsamples), skiprows=1, header=None
                                  )
...
if cosmology == "oLCDM":
    fig = corner.corner(
        ...
        title_fmt='.3f', use_math_text=True, truths=[H0_mock, Om0_mock, Ok0_mock]
    )
    fig.suptitle('Corner plot, %s LSST-like samples, mock=%s,%s,%s,%s' 
        % (len(zlens), H0_mock, Om0_mock, Ok0_mock, cosmology), y=1.02)
    fig.savefig("Plots/%sLSSTLike_mock=%s,%s,%s,%s_%ix%i_corner.png" 
                % (len(zlens), H0_mock, Om0_mock, Ok0_mock, cosmology, nwalkers, nsamples)
                , bbox_inches='tight'
               )
elif cosmology == "owCDM":
    fig = corner.corner(
        flat_samples, labels=labels, quantiles=(0.16, 0.84), show_titles=True, 
        title_fmt='.3f', use_math_text=True, truths=[H0_mock, Om0_mock, Ok0_mock, w_mock]
    )
    fig.suptitle('Corner plot, %s LSST-like samples, mock=%s,%s,%s,%s,%s' 
        % (len(zlens), H0_mock, Om0_mock, Ok0_mock, w_mock, cosmology), y=1.02)
    fig.savefig("Plots/%sLSSTLike_mock=%s,%s,%s,%s,%s_%ix%i_corner.png" 
                % (len(zlens), H0_mock, Om0_mock, Ok0_mock, w_mock, cosmology, nwalkers, nsamples)
                , bbox_inches='tight'
               )
